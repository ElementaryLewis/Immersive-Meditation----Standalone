...
class CPlayerInput 
{
	...
	{
		var imInstalled : bool; //Immersive Meditation
	...
	event OnIngameMenu( action : SInputAction )
	{
			...
			&& FactsQuerySum("nge_pause_menu_disabled") <= 0 ) 
		{
			if( ((CMeditationUI)thePlayer.getMeditation()).getMedMenuBool() && imInstalled )
			{
				...
				{
					((W3Campfire)((CMeditationUI)thePlayer.getMeditation()).GetCampFire()).AddTimer('UnLight',1.7,false);
				}
				else
				{
					((W3Campfire)((CMeditationUI)thePlayer.getMeditation()).GetCampFire()).AddTimer('UnLight',(8.7-passedSecondsInRealTime),false);
				}
			}
			else if( IsReleased(action) && openedPanel != 'GlossaryTutorialsMenu' && !theGame.GetGuiManager().IsAnyMenu() ) // #B very ugly :P
			{
				if ( theGame.IsBlackscreenOrFading() )
					return false;
