...
state Meditation in W3PlayerWitcher extends MeditationBase
{
	...
	event OnEnterState( prevStateName : name )
	{
		...
		medVec = Vector( parent.mImmersive.medOffset, parent.mImmersive.medDepth, parent.mImmersive.medHeight);
		// ImmersiveMeditation--
	...
	entry function InitState(prevStateName : name)
	{
		...
		if(prevStateName != 'MeditationWaiting')
		{
			...
			if ( parent.IsInInterior() || !parent.mImmersive.useCampfire )
				isOutside = 0;
			else
				isOutside = 1;
			//Immersive Meditation--
			if( !((W3WitcherBed)theGame.GetEntityByTag( 'witcherBed' )).GetWasUsed() )
			{
				...
				virtual_parent.SetBehaviorVariable('MeditateWithIgnite', isOutside);	//Immersive Meditation
	...
	public function StopRequested(optional closeUI : bool)
	{
		...
		virtual_parent.SetBehaviorVariable('MeditateAbort', 0); //Immersive Meditation
	}
	...
	public latent function StopMeditation()
	{
		...
		virtual_parent.SetBehaviorVariable('HasCampfire', isOutside ); //Immersive Meditation
	...
	event OnGameCameraTick( out moveData : SCameraMovementData, dt : float )
	{
		...
		if(!cameraIsLeavingState)
		{
			...
			parent.UpdateCameraMeditation( moveData, dt );
			moveData.pivotRotationController.SetDesiredPitch( parent.mImmersive.medPitch, 0.2 );
			moveData.pivotRotationController.SetDesiredHeading( rotation.Yaw + parent.mImmersive.medEndFacing, parent.mImmersive.medRotSpeed );
		...
		else
		{
			if(closeCamera == "1")
			{
				...
				DampVectorSpring( medVec, moveData.cameraLocalSpaceOffsetVel, Vector( 0.65f, 0.3f, 0.6f ), 0.6f, dt );
			}
			else if(!isOutside && !thePlayer.mImmersive.noInteriorCamChange )
			{
				DampVectorSpring( medVec, moveData.cameraLocalSpaceOffsetVel, Vector( thePlayer.mImmersive.intOffset, thePlayer.mImmersive.intDepth, thePlayer.mImmersive.intHeight ), 0.6f, dt );
			}
			else
			{
				DampVectorSpring( medVec, moveData.cameraLocalSpaceOffsetVel, Vector( thePlayer.mImmersive.expOffset, thePlayer.mImmersive.expDepth, thePlayer.mImmersive.expHeight ), 0.6f, dt );
			}
		}
		// ImmersiveMeditation--
		return false;
